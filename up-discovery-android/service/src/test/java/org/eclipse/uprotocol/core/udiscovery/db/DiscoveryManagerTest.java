...
public class DiscoveryManagerTest extends TestBase implements PersistInterface {
    ...
    public void setUp(){
        ...
        mPersistFlag = false;
        mEntityV1 = UEntity.newBuilder().setName(TEST_ENTITY).setVersionMajor(1).build();
        mEntityV2 = UEntity.newBuilder().setName(TEST_ENTITY).setVersionMajor(2).build();
    ...
    public void positive_lookupuri_authority() {
        UEntity entity = UEntity.newBuilder().setName(Constants.JSON_AUTHORITY).build();
        UUri uri = UUri.newBuilder().setEntity(entity).build();
    ...
    public void negative_update_device_node_missing_domain() {
        ...
        bld.setUri(Utils.toLongUri(rootUri));
    ...
    public void negative_update_device_node_bad_domain() {
        ...
        String uri = Utils.toLongUri(authority);
        childBld.setUri(uri);
        childBld.setType(Node.Type.DEVICE);
    ...
    public void negative_load_invalid_authority() {
        ...
        String original = "\"" + Constants.JSON_AUTHORITY + "\": \"" + TEST_AUTHORITY.getName() + "\"";
        String replacement = "\"" + Constants.JSON_AUTHORITY + "\": \"corrupt data\"";
    ...
    public void negative_load_hash_check_failure() {
        ...
        String corruptJson = validJson.replace(TEST_ENTITY, "");
    ...
    public void positive_ttl() throws InterruptedException {
        ...
        UEntity entity = UEntity.newBuilder().setName(TEST_ENTITY).build();
        UUri uri = UUri.newBuilder().setAuthority(TEST_AUTHORITY).setEntity(entity).build();
        Node nodeToExpire = Node.newBuilder().setUri(Utils.toLongUri(uri)).setType(Node.Type.ENTITY).build();
    ...
    public void negative_save_load_expired_ttl() throws InterruptedException {
        ...
        String target = Utils.toLongUri(uri) + "\":";
        ...
        String replacement = Utils.toLongUri(corruptUri) + "\":";
    ...
    public void positive_deletenode_different_path() {
        populateDatabase();
        UEntity entity1 = UEntity.newBuilder().setName(TEST_ENTITY).build();
        ...
        Node node = Node.newBuilder().setUri(Utils.toLongUri(uri2)).setType(Node.Type.ENTITY).build();
    ...
    public void negative_resource_level_authority_mismatch() {
        ...
        UUri child = UUri.newBuilder().setAuthority(TEST_AUTHORITY).setEntity(mEntityV1).setResource(
                mNewResource).build();
        String versionUri = Utils.toLongUri(parent);
        String topicUri = Utils.toLongUri(child);
    ...
    public void negative_version_level_authority_mismatch() {
        ...
        String entityUri = Utils.toLongUri(parentUri);
        String verionUri = Utils.toLongUri(childUri);
        Node versionNode = Node.newBuilder()
                .setUri(verionUri)
                .setType(Node.Type.VERSION)
                .build();
    ...
    public void positive_addNodes_add_device() {
        ...
        Node node = Node.newBuilder()
                .setUri(Utils.toLongUri(deviceUri))
    ...
    public void positive_addNodes_add_entity() {
        ...
        Node node = Node.newBuilder()
                .setUri(Utils.toLongUri(resourceUri))
    ...
    public void negative_addNodes_already_exists() {
        ...
        Node duplicateNode = Node.newBuilder()
                .setUri(Utils.toLongUri(entityUri))
                .setType(Node.Type.ENTITY)
                .build();
        UUri parentUri = UUri.newBuilder().setAuthority(TEST_AUTHORITY).build();

...
import org.eclipse.uprotocol.core.udiscovery.db.JsonNodeTest;
...
public class RPCHandlerTest extends TestBase {
    ...
    public void initializeDb() {
        ...
        Node node = jsonToNode(JsonNodeTest.REGISTRY_JSON);
    ...
    public void positive_persist() {
        ...
        verify(mAssetUtil).writeFileToInternalStorage(mContext, Constants.LDS_DB_FILENAME, payload);
    }

    @Test
    public void positive_processLookupUri() throws InvalidProtocolBufferException {
        UEntity entity = UEntity.newBuilder().setName(TEST_ENTITY).build();
    ...
    public void positive_processFindNodes() throws InvalidProtocolBufferException {
        ...
        FindNodesRequest request = FindNodesRequest.newBuilder().setUri(Utils.toLongUri(uri)).build();
    ...
    public void positive_processFindNodes_debug() throws InvalidProtocolBufferException {
        setLogLevel(Log.DEBUG);
        UUri uri = UUri.newBuilder().setAuthority(TEST_AUTHORITY).build();
        FindNodesRequest request = FindNodesRequest.newBuilder().setUri(Utils.toLongUri(uri)).setDepth(0).build();
        UMessage uMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
    ...
    public void positive_processUpdateNode() throws InvalidProtocolBufferException {
        UEntity entity = UEntity.newBuilder().setName(TEST_ENTITY).build();
        UUri uri = UUri.newBuilder().setAuthority(TEST_AUTHORITY).setEntity(entity).build();
        Node node = Node.newBuilder().setUri(Utils.toLongUri(uri)).setType(Node.Type.ENTITY).build();
        ...
        verify(mAssetUtil).writeFileToInternalStorage(eq(mContext), eq(Constants.LDS_DB_FILENAME), anyString());
        assertEquals(UCode.OK, status.getCode());
    }
    ...
    public void positive_processAddNodes() throws InvalidProtocolBufferException {
        ...
        AddNodesRequest request = AddNodesRequest.newBuilder()
                .setParentUri(Utils.toLongUri(parentUri))
                .addNodes(node)
                .build();
        UMessage uMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
    ...
    public void positive_processDeleteNodes() throws InvalidProtocolBufferException {
        UEntity entity = UEntity.newBuilder().setName(TEST_ENTITY).build();
        UUri uri = UUri.newBuilder().setAuthority(TEST_AUTHORITY).setEntity(entity).build();
        DeleteNodesRequest request = DeleteNodesRequest.newBuilder().addUris(Utils.toLongUri(uri)).build();
        UMessage uMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
    ...
    public void positive_RegisterForNotifications() throws InvalidProtocolBufferException {
        ...
        ObserverInfo info = ObserverInfo.newBuilder().setUri(Utils.toLongUri(ObserverUri)).build();
        NotificationsRequest request = NotificationsRequest.newBuilder()
                .setObserver(info)
                .addUris(Utils.toLongUri(nodeUri))
                .build();
        UMessage uMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
    ...
    public void negative_refreshDatabase() throws InvalidProtocolBufferException {
        ...
        UPayload uPayload = mRPCHandler.processLDSUpdateNode(uMsg);
        Any any = Any.pack(UMessage.newBuilder().setPayload(uPayload).build());
    ...
    @Test
    public void positive_processUpdatePropertyCheckPermission() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUInteger(10).build();

        UpdatePropertyRequest.Builder reqBld = UpdatePropertyRequest.newBuilder();
        reqBld.setUri("ultifi:/body.access/1/").setProperty("permission_level").setValue(propertyValue);
        UpdatePropertyRequest request = reqBld.build();
        UMessage updateMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();

        UPayload uPayload = mRPCHandler.processLDSUpdateProperty(updateMsg);
        UStatus sts = unpack(uPayload, UStatus.class).
                orElseThrow(() -> new UStatusException(UCode.INVALID_ARGUMENT, UNEXPECTED_PAYLOAD));

        assertEquals(UCode.OK_VALUE, sts.getCode());
        verify(mAssetUtil, times(1)).writeFileToInternalStorage(eq(mContext), eq(Constants.LDS_DB_FILENAME), any(String.class));
    }

    @Test
    public void negative_processUpdatePropertyCheckPermission() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUInteger(10).build();
        ...
        assertEquals(UCode.PERMISSION_DENIED_VALUE, sts.getCode());
        verify(mAssetUtil, times(0)).writeFileToInternalStorage(eq(mContext), eq(Constants.LDS_DB_FILENAME), any(String.class));
    }

    @Test
    public void positive_processUpdatePropertyCheckPermissionUOTA() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUInteger(10).build();
...
//    @Ignore
    @Test
    public void positive_processUpdatePropertyCheckPermissionBOdyAccess() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUBoolean(false).build();

        UpdatePropertyRequest.Builder reqBld = UpdatePropertyRequest.newBuilder();
        reqBld.setUri("ultifi:/body.access/1/door.front_left").setProperty("is_driver_door").setValue(propertyValue);
        UpdatePropertyRequest request = reqBld.build();
        UMessage updateMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
    ...
    @Test
    public void positive_processUpdatePropertyCheckPermissionTESTAPP() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUBoolean(false).build();

        UpdatePropertyRequest.Builder reqBld = UpdatePropertyRequest.newBuilder();
        reqBld.setUri("ultifi:/core.test/1/rpc.ResoveUri").setProperty("id").setValue(propertyValue);
        UpdatePropertyRequest request = reqBld.build();
        UMessage updateMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
...
//    @Ignore
    @Test
    public void negative_processUpdatePropertyCheckPermissionBOdyAccess() throws InvalidProtocolBufferException {
            setLogLevel(Log.VERBOSE);
            PropertyValue propertyValue = PropertyValue.newBuilder().setUBoolean(false).build();

            UpdatePropertyRequest.Builder reqBld = UpdatePropertyRequest.newBuilder();
            reqBld.setUri("ultifi:/body.access/1").setProperty("ent_prop1").setValue(propertyValue);
            UpdatePropertyRequest request = reqBld.build();
            UMessage updateMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
            ...
            assertEquals(UCode.PERMISSION_DENIED_VALUE, sts.getCode());
            verify(mAssetUtil, times(0))
                    .writeFileToInternalStorage(eq(mContext), eq(Constants.LDS_DB_FILENAME), any(String.class));
        }

    @Test
    public void negative_processUpdatePropertyUENotAuthorized() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUBoolean(false).build();

        UpdatePropertyRequest.Builder reqBld = UpdatePropertyRequest.newBuilder();
        reqBld.setUri("ultifi:/core.test/1").setProperty("major_version").setValue(propertyValue);
        UpdatePropertyRequest request = reqBld.build();
        UMessage updateMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
    ...
    @Test
    public void negative_processUpdatePropertyTestAppOtherUE() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUBoolean(false).build();

        UpdatePropertyRequest.Builder reqBld = UpdatePropertyRequest.newBuilder();
        reqBld.setUri("ultifi:/body.access/1").setProperty("major_version").setValue(propertyValue);
        UpdatePropertyRequest request = reqBld.build();
        UMessage updateMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();
    ...
    @Test
    public void negative_processUpdatePropertyUDiscoveryRemote() throws InvalidProtocolBufferException {
        setLogLevel(Log.VERBOSE);
        PropertyValue propertyValue = PropertyValue.newBuilder().setUBoolean(false).build();
    ...
    @Test
    public void positive_processAddNodesTypeMESSAGE() throws InvalidProtocolBufferException {
        Node.Builder nodeBld = Node.newBuilder();
        nodeBld.setUri("ultifi:/body.access/1/door.rear_left");
        nodeBld.setType(Node.Type.MESSAGE);

        AddNodesRequest.Builder reqBld = AddNodesRequest.newBuilder();
        reqBld.setParentUri("ultifi:/body.access/1");
        reqBld.addNodes(nodeBld);
        AddNodesRequest request = reqBld.build();
        UMessage addMsg = UMessage.newBuilder().setPayload(packToAny(request)).build();

        UPayload uPayload = mRPCHandler.processAddNodesLDS(addMsg);
        UStatus sts = unpack(uPayload, UStatus.class).
                orElseThrow(() -> new UStatusException(UCode.INVALID_ARGUMENT, UNEXPECTED_PAYLOAD));
    ...
    @Test
    public void positive_processAddNodesTypeRESOURCE() throws InvalidProtocolBufferException {
        Node.Builder nodeBld = Node.newBuilder();
        nodeBld.setUri("ultifi:/body.access/1/door.rear_left");
        nodeBld.setType(Node.Type.RESOURCE);

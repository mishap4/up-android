...
public class DatabaseUtilityTest {
    ...
    public void setUp() {
        ...
        String domain = Utils.parseAuthority(TestBase.TEST_AUTHORITY).second;
        ...
        UResource resource = UResource.newBuilder().setName(TestBase.TEST_RESOURCE).setInstance(TestBase.TEST_INSTANCE_4).build();

        mLevel0 = Utils.toLongUri(UUri.newBuilder().setAuthority(rootAuthority).build());
        mLevel1 = Utils.toLongUri(UUri.newBuilder().setAuthority(TestBase.TEST_AUTHORITY).build());
        String level2_0 = Utils.toLongUri(UUri.newBuilder().setAuthority(TestBase.TEST_AUTHORITY).setEntity(entityShort).build());
        String level2_1 = Utils.toLongUri(UUri.newBuilder().setAuthority(TestBase.TEST_AUTHORITY).setEntity(entityLong).build());
        mLevel2 = Utils.toLongUri(UUri.newBuilder().setAuthority(TestBase.TEST_AUTHORITY).setEntity(entityMedium).build());
        mLevel3 = Utils.toLongUri(UUri.newBuilder().setAuthority(TestBase.TEST_AUTHORITY).setEntity(entityV1).build());
        mLevel4 = Utils.toLongUri(UUri.newBuilder().setAuthority(TestBase.TEST_AUTHORITY).setEntity(entityV1).setResource(resource).build());
    ...
    public void isSubUri_test() {
        String uri = Utils.toLongUri(UUri.newBuilder().setAuthority(TestBase.TEST_AUTHORITY).build());

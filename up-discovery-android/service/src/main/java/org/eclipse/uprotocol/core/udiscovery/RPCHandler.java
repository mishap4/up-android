...
public class RPCHandler implements PersistInterface {
    ...
    public UPayload processLookupUriFromLDS(@NonNull UMessage uMsg) {
        ...
        try {
            ...
            if (DEBUG) {
                Log.d(LOG_TAG, join(Key.REQUEST, "LookupUri", Key.URI, Utils.toLongUri(uri)));
            }
            ...
            final UStatus status = errorStatus(LOG_TAG, "LookupUri", Utils.throwableToStatus(e));
    ...
    public UPayload processFindNodesFromLDS(@NonNull UMessage uMsg) {
        ...
        try {
            ...
            final UStatus status = errorStatus(LOG_TAG, "FindNodes", Utils.throwableToStatus(e));
    ...
    public UPayload processFindNodeProperties(@NonNull UMessage uMsg) {
        ...
        try {
            ...
            final UStatus status = errorStatus(LOG_TAG, "FindNodeProperties", Utils.throwableToStatus(e));
    ...
    public UPayload processLDSUpdateNode(@NonNull UMessage uMsg) {
        ...
        try {
            ...
            status = errorStatus(LOG_TAG, "UpdateNode", Utils.throwableToStatus(e));
        }
    ...
    public UPayload processLDSUpdateProperty(@NonNull UMessage uMsg) {
        ...
        try {
            ...
            status = errorStatus(LOG_TAG, "UpdateNodeProperty", Utils.throwableToStatus(e));
        }
        return packToAny(status);
    }
    ...
    public UPayload processAddNodesLDS(@NonNull UMessage uMsg) {
        ...
        try {
            ...
            status = errorStatus(LOG_TAG, "AddNodes", Utils.throwableToStatus(e));
        }
        return packToAny(status);
    }
    ...
    public UPayload processDeleteNodes(@NonNull UMessage uMsg) {
        ...
        try {
            ...
            final List<UUri> nodeUriList = Utils.deserializeUriList(urisList);
            if (DEBUG) {
            ...
            status = errorStatus(LOG_TAG, "DeleteNodes", Utils.throwableToStatus(e));
        }
        return packToAny(status);
    }
    ...
    public UPayload processNotificationRegistration(@NonNull UMessage uMsg, String methodName) {
        ...
        try {
            ...
            final List<UUri> nodeUriList = Utils.deserializeUriList(request.getUrisList());
            if (UDiscoveryService.METHOD_REGISTER_FOR_NOTIFICATIONS.equals(methodName)) {
                ...
                throw new Utils.StatusException(code, message);
            }
        } catch (Exception e) {
            status = errorStatus(LOG_TAG, "processNotificationRegistration", Utils.throwableToStatus(e));
        }
        return packToAny(status);
    }
    ...
    private UStatus registerNotifications(@NonNull UUri observer, @NonNull List<UUri> nodeUriList) {
        if (DEBUG) {
            Log.d(LOG_TAG, join(Key.REQUEST, "RegisterForNotifications",
                    OBSERVER_URI, Utils.toLongUri(observer), URILIST, nodeUriList));
        }

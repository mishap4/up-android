...
public class Notifier {
    ...
    private final ULink mUltifiLink;
        ...
        observerList.forEach((observer) -> {
            if (DEBUG) {
                Log.d(LOG_TAG, join(Key.MESSAGE, "notify",
                        Key.URI, Utils.toLongUri(child),
                        PARENT_URI, Utils.toLongUri(parent),
    ...
    private void buildNotifications(UUri nodeUri, UUri parentUri, Operation operation,
            UUri observerUri) {
        final String child = Utils.toLongUri(nodeUri);
        final String parent = Utils.toLongUri(parentUri);

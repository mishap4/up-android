...
public class ObserverManager {
    ...
    public UStatus registerObserver(@NonNull List<UUri> nodeUris, UUri observerUri) {
        ...
        if (addedCount == nodeUris.size()) {
            return Utils.buildStatus(UCode.OK);
        } else {
            //TODO: check with Cloud team to retry request if partial success case
            return Utils.buildStatus(UCode.INTERNAL, "Operation didn't complete for all Nodes");
        }
    }
        ...
        mObservers.compute(nodeUri, (key, value) -> {
            ...
            if (DEBUG) {
                Log.d(LOG_TAG, join(Key.EVENT, "addObserverToMap",
                        NODEURI, Utils.toLongUri(nodeUri),
                        OBSERVER_URI, Utils.toLongUri(observer)));
            }
            return observers;
        });
        ...
        synchronized (mDBLock) {
            try {
                long retCode = observerDao().addObserver(
                        new Observer(Utils.toLongUri(nodeUri), Utils.toLongUri(observerUri)));
                if (retCode < 0) {
                    Log.e(LOG_TAG, join(Key.MESSAGE, "Failed to add to DB with code", Key.CODE, retCode));
                }
                return retCode >= 0;
            } catch (Exception e) {
                errorStatus(LOG_TAG, "addObserverToDb", Utils.throwableToStatus(e),Key.MESSAGE, "Failed to add to DB");
                return false;
            }
        }
    }
    ...
    private boolean removeObserverFromDb(@NonNull UUri nodeUri, @NonNull UUri observerUri) {
        synchronized (mDBLock) {
            try {
                int retCode = observerDao().removeObserver(Utils.toLongUri(nodeUri), Utils.toLongUri(observerUri));
                if (retCode < 0) {
                    Log.e(LOG_TAG, join(Key.EVENT, "Failed to remove from DB with code", Key.CODE, retCode));
                }
                return retCode >= 0;
            } catch (Exception e) {
                errorStatus(LOG_TAG, "removeObserverFromDb", Utils.throwableToStatus(e), Key.MESSAGE, "Failed to remove from DB");
                return false;
            }
        }
    }
        ...
        if (VERBOSE) {
            ...
            for (Map.Entry<UUri, Set<UUri>> entry : observersMap.entrySet()) {
                for (UUri observer : entry.getValue()) {
                    String key = Utils.toLongUri(entry.getKey());
                    String val = Utils.toLongUri(observer);

...
public class DatabaseUtility {
    ...
    public static void DeleteNodeFromPath(List<Node.Builder> nodePath) {
        ...
        Utils.checkArgument(rootNodeSize >= 2,
    ...
    private static boolean isSubUri(@NonNull String uri, @NonNull String subUri) {
        if (uri.startsWith(subUri)) {
            ...
            return (uri.length() == length) ? true : Utils.hasCharAt(uri, length, '/');
        }
        return false;
    }
    ...
    public static Node internalDeleteNode(Node node, String uri) {
        Utils.checkNotNull(node, "[internalDeleteNode] node is null");
        Utils.checkStringNotEmpty(uri, "[internalDeleteNode] uri is empty");
        final List<Node.Builder> nodePath = FindPathToNode(node.toBuilder(), uri);
        Utils.checkArgumentPositive(nodePath.size(), UCode.NOT_FOUND,  "[deleteNode] could not find " + uri);
    ...
    public static Node verifyNode(Node node) {
        Utils.checkNotNull(node, "[verifyNode] node is null");
        ...
        while(!stack.isEmpty()) {
            ...
            Utils.checkArgument(isValidType, "[verifyNode] invalid node type " + bld.getType());
            final String uri = Utils.sanitizeUri(bld.getUri());
            bld.setUri(uri);
            verifyLineage(bld);
    ...
    public static Node copy(Node node, int depth) {
        Utils.checkNotNull(node, "[copy] node is null");
        final Node.Builder rootBld = node.toBuilder();
    ...
    public static Node.Builder commitNode(Node.Builder bld, Node node) {
        Utils.checkNotNull(bld, "[commitNode] bld is null");
        Utils.checkNotNull(node, "[commitNode] node is null");
    ...
    private static void verifyLineage(Node.Builder parent) {
        Utils.checkNotNull(parent, "[verifyLineage] parent is null");
        final String parentUri = parent.getUri();
        final Set<String> uriList = new HashSet<>();
        for (Node child : parent.getNodesList()) {
            final String childUri = Utils.sanitizeUri(child.getUri());
            Utils.checkArgument(uriList.add(childUri), UCode.ALREADY_EXISTS,
                    "[verifyLineage] node already exists " + childUri);
            final boolean result = verifyParentChild(parentUri, childUri);
            Utils.checkArgument(result, "[verifyLineage] invalid hierarchy: " +
    ...
    private static boolean verifyParentChild(String parent, String child) {
        try {
            ...
            Utils.checkArgumentPositive(parentList.size(), "invalid parent uri " + parent);
            Utils.checkArgumentPositive(childList.size(), "invalid child uri " + child);
    ...
    private static boolean verifyDomainDevice(String parent, String child) {
        ...
        final String childDomain = Utils.parseAuthority(childAuthority).second;

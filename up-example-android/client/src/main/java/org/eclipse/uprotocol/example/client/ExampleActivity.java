...
public class ExampleActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mULink = ULink.create(getApplicationContext(), getMainExecutor(), (ulink, ready) -> {
            if (ready) {
                mLog.i(TAG, join(Key.EVENT, "uLink connected"));
            } else {
                mLog.w(TAG, join(Key.EVENT, "uLink unexpectedly disconnected"));
            }
        });

        mUSubscriptionStub = USubscription.newStub(mULink);
        mExampleStub = Example.newStub(mULink);
    ...
    protected void onDestroy() {
        ...
        CompletableFuture.allOf(
                ...
                .thenCompose(it -> mULink.disconnect())

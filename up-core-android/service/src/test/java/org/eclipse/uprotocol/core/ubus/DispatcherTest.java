...
public class DispatcherTest extends TestBase {
    ...
    private void verifyMessageReceived(UMessage message, int times, @NonNull Client client) {
        if (message != null) {
            verify(((UListener)client.getListener()), timeout(DELAY_MS).times(times)).onReceive(message);
        } else {
            verify(((UListener)client.getListener()), timeout(DELAY_MS).times(times)).onReceive(any());
        }
    }
